...
void LineNumbers::setFont(const QFont &font)
{
    ...
    font_ = font;
    font_.setPointSize(fontPointSize_);
    line_height_ = QFontMetrics{font_}.height();
    update();
}

void LineNumbers::setFontPointSize(int &fontPointSize){
...
{
    int lines = 1;
    lines += std::count_if(std::begin(text), std::end(text), [](const QChar &symbol) {
        return (symbol == '\n' || symbol == '\r');
    });
    return lines;
}
